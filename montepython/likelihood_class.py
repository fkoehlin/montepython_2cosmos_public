...
class Likelihood(object):
    ...
    def need_cosmo1_arguments(self, data, dictionary):
        """
        ...
        for key, value in dictitems(dictionary):
            try:
                data.cosmo1_arguments[key]
                try:
                    float(data.cosmo1_arguments[key])
                    num_flag = True
                except ValueError:
                    num_flag = False
                except TypeError:
            ...
            if num_flag is False:
                if data.cosmo1_arguments[key].find(value) == -1:
                    data.cosmo1_arguments[key] += ' '+value+' '
            else:
                if array_flag is False:
                    if float(data.cosmo1_arguments[key]) < value:
                        data.cosmo1_arguments[key] = value
                else:
                    data.cosmo1_arguments[key] = '%.2g' % value[0]
                    for i in range(1, len(value)):
                        data.cosmo1_arguments[key] += ',%.2g' % (value[i])

    def need_cosmo2_arguments(self, data, dictionary):
        """
        Ensure that the arguments of dictionary are defined to the correct
        value in the cosmological code
        ...
        for key, value in dictitems(dictionary):
            try:
                data.cosmo2_arguments[key]
                try:
                    float(data.cosmo2_arguments[key])
                    num_flag = True
                except ValueError:
                    num_flag = False
                except TypeError:
                ...
                try:
                    ...
                    array_flag = True
            if num_flag is False:
                if data.cosmo2_arguments[key].find(value) == -1:
                    data.cosmo2_arguments[key] += ' '+value+' '
            else:
                if array_flag is False:
                    if float(data.cosmo2_arguments[key]) < value:
                        data.cosmo2_arguments[key] = value
                else:
                    data.cosmo2_arguments[key] = '%.2g' % value[0]
                    for i in range(1, len(value)):
                        data.cosmo2_arguments[key] += ',%.2g' % (value[i])
    ...
    def computeLikelihood(self, ctx):
        ...
        cosmo1 = ctx.get("cosmo1")
        cosmo2 = ctx.get("cosmo2")
        data = ctx.get("data")

        loglkl = self.loglkl(cosmo1, cosmo2, data)
...
### FK: likelihoods below NOT modified to be compatible with 2cosmos modifications!!! ###
